...
public class Minesweeper implements ActionListener{
	...
	public Minesweeper() {
		...
		// If the user selects 32x32
			...
			jframe.setSize(32*20, 32*20);
			colmines = 32;
			rowmines = 32;
		}
		
		// If the user selects 64x63
		else if (s != null && s.length() > 0 && s.equals("64x64")) {
			jframe.setSize(64*20, 64*20);
			colmines = 64;
			rowmines = 64;
		}
		...
		for (int x = 0; x < counts.length; x++) {
			for (int y = 0; y <counts[0].length; y++) {
				if (counts[x][y] != mine) {
					...
					if (y > 0 && counts [x][y-1] == mine) {// up X
						neighbormines++;
					}
					
					if (x < counts.length - 1 && y > 0 && counts[x+1][y-1] == mine) {// down right
						neighbormines++;
					}
					
					if (x > 0 && counts[x-1][y] == mine) { // left X
						neighbormines++;
					}
					
					if (x < counts.length - 1 && counts [x+1][y] == mine) { // right X
						neighbormines++;
					}
					
					if (x > 0 && y < counts[0].length - 1 && counts[x-1][y+1] == mine) { // upright
						neighbormines++;
					}
					
					if (y < counts[0].length - 1 && counts[x][y+1] == mine) { //down X
						neighbormines++;
					}
